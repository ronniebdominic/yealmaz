...
// the ORIGINAL case's totalAmount). Structural mirror of
// TeamLeaveRequests.jsx (same hideEmpty convention, same
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Original Case</th><th>Reason</th><th style={{ textAlign: 'right' }}>Actions</th>
          </tr>
        </thead>
        <tbody>
          {cases.map(c => {
            const redoAmount = c.originalCase?.totalAmount != null
              ? Math.round(c.originalCase.totalAmount * 0.5 * 100) / 100
              : null;
            return (
              <tr key={c.id}>
                ...
                <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={() => decide(c.id, 'REDO')} disabled={!c.originalCase}
                    title={redoAmount != null ? `Charges Br ${redoAmount.toLocaleString('en-US')}` : 'No original case linked'}>

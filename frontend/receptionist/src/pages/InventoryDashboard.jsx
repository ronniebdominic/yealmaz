  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 420, overflow: 'hidden' }}>
        <div style={{ background: 'var(--amber)', color: '#fff', padding: '14px 18px', fontWeight: 700, fontSize: 15, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>ITEM NAME *</div>
            <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
              placeholder="e.g. Zirconia Blank" style={inputStyle} />
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>UNIT *</div>
              <input value={form.unit} onChange={e => setForm(p => ({ ...p, unit: e.target.value }))}
                placeholder="pcs, box, kg…" style={inputStyle} />
            </div>
            {!isEdit && (
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>STARTING QTY</div>
                <input type="number" min="0" value={form.quantityOnHand} onChange={e => setForm(p => ({ ...p, quantityOnHand: e.target.value }))} style={inputStyle} />
              </div>
            )}
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>LOW-STOCK ALERT THRESHOLD</div>
          ...
          <button onClick={save} disabled={saving}
            style={{ marginTop: 6, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--amber)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: saving ? 'not-allowed' : 'pointer' }}>
...
function AdjustModal({ item, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 420, overflow: 'hidden' }}>
        <div style={{ background: 'var(--navy)', color: '#fff', padding: '14px 18px', fontWeight: 700, fontSize: 15, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          Adjust Stock — {item.name}
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer' }}><MdClose size={18} /></button>
        </div>
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Currently on hand: <strong style={{ color: 'var(--text-1)' }}>{item.quantityOnHand} {item.unit}</strong></div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setType('RESTOCK')} style={{ ...pillStyle, ...(type === 'RESTOCK' ? { background: '#DCFCE7', color: '#166534', border: '1.5px solid var(--green)' } : {}) }}>+ Restock</button>
            <button onClick={() => setType('ADJUSTMENT')} style={{ ...pillStyle, ...(type === 'ADJUSTMENT' ? { background: 'var(--amber-dim)', color: 'var(--amber)', border: '1.5px solid var(--amber)' } : {}) }}>± Correction</button>
          </div>
          <div>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>NOTE</div>
            <input value={note} onChange={e => setNote(e.target.value)} placeholder="Reason (optional)" style={inputStyle} />
          </div>
          <button onClick={save} disabled={saving}
            style={{ marginTop: 6, padding: '10px', borderRadius: 8, border: 'none', background: 'var(--navy)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: saving ? 'not-allowed' : 'pointer' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 440, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ background: 'var(--bg)', borderRadius: 10, padding: '12px 14px', border: '1px solid var(--border)', fontSize: 13 }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{request.item.name} <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>({request.quantityRequested} {request.item.unit})</span></div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setAction('ACCEPT')} style={{ ...pillStyle, ...(action === 'ACCEPT' ? { background: '#DCFCE7', color: '#166534', border: '1.5px solid var(--green)' } : {}) }}><MdCheckCircle size={14} /> Accept</button>
            <button onClick={() => setAction('REJECT')} style={{ ...pillStyle, ...(action === 'REJECT' ? { background: '#FEE2E2', color: '#991B1B', border: '1.5px solid var(--red)' } : {}) }}><MdCancel size={14} /> Reject</button>
          </div>
          {action === 'ACCEPT' && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 4 }}>QUANTITY TO ISSUE</div>
          ...
          <button onClick={save} disabled={saving}
            style={{ marginTop: 6, padding: '10px', borderRadius: 8, border: 'none', background: action === 'ACCEPT' ? 'var(--green)' : 'var(--red)', color: '#fff', fontWeight: 700, fontSize: 13, cursor: saving ? 'not-allowed' : 'pointer' }}>
...
const pillStyle = { flex: 1, padding: '8px 10px', borderRadius: 8, border: '1px solid #D1D5DB', background: 'var(--bg)', color: 'var(--text-2)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5 };
...
export default function InventoryDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'Manrope, sans-serif' }}>
      <div style={{ background: 'var(--navy)', color: '#fff', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src="/logo.png" alt="logo" style={{ width: 34, height: 34, borderRadius: '50%', objectFit: 'cover' }} />
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, letterSpacing: 0.2 }}>Ye-Almaz Dental Lab</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--amber)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>{initials}</div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px 16px' }}>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{
                ...
                border: tab === t.id ? '1.5px solid var(--amber)' : '1px solid var(--border)',
                background: tab === t.id ? 'var(--amber-dim)' : '#fff', color: tab === t.id ? 'var(--amber)' : 'var(--text-3)',
              }}>
              <t.icon size={15} /> {t.label}
              {t.id === 'requests' && pendingCount > 0 && (
                <span style={{ background: 'var(--red)', color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: 11 }}>{pendingCount}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {tab === 'stock' && (
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontWeight: 700, fontSize: 14 }}>Stock Items</div>
              <button onClick={() => setItemModal({})}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 8, border: 'none', background: 'var(--amber)', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                <MdAdd size={15} /> Add Item
              </button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--bg)', fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase' }}>
              ...
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={4} style={{ padding: 30, textAlign: 'center', color: 'var(--text-4)' }}>No items yet — add one above</td></tr>
                ) : items.map(i => {
                  const low = i.reorderThreshold != null && i.quantityOnHand <= i.reorderThreshold;
                  return (
                    <tr key={i.id} style={{ borderTop: '1px solid var(--surface-2)', background: low ? '#FEF2F2' : 'transparent' }}>
                      <td style={{ padding: '10px 14px', fontWeight: 600 }}>{i.name} <span style={{ fontWeight: 400, color: 'var(--text-4)', fontSize: 12 }}>({i.unit})</span></td>
                      <td style={{ padding: '10px 14px', textAlign: 'center', fontWeight: 700, color: low ? 'var(--red)' : 'var(--text-1)' }}>
                        {i.quantityOnHand} {low && <MdWarning size={13} style={{ verticalAlign: 'middle', marginLeft: 3 }} />}
                      </td>
                      <td style={{ padding: '10px 14px', textAlign: 'center', color: 'var(--text-3)' }}>{i.reorderThreshold ?? '—'}</td>
        ...
        {tab === 'requests' && (
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
              {['PENDING', 'FULFILLED', 'REJECTED', 'ALL'].map(s => (
                <button key={s} onClick={() => setRequestStatusFilter(s)}
                  style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700, cursor: 'pointer', border: '1px solid var(--border)', background: requestStatusFilter === s ? 'var(--navy)' : '#fff', color: requestStatusFilter === s ? '#fff' : 'var(--text-3)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {requests.length === 0 ? (
                <div style={{ padding: 30, textAlign: 'center', color: 'var(--text-4)', background: '#fff', borderRadius: 12, border: '1px solid var(--border)' }}>No requests here</div>
              ) : requests.map(r => (
                <div key={r.id} style={{ background: '#fff', borderRadius: 12, border: '1px solid var(--border)', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>{r.item?.name} <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>× {r.quantityRequested} {r.item?.unit}</span></div>
                    ...
                    {r.status !== 'PENDING' && (
                      <div style={{ fontSize: 12, marginTop: 4, fontWeight: 700, color: r.status === 'FULFILLED' ? 'var(--green)' : 'var(--red)' }}>
                  ...
                  {r.status === 'PENDING' && (
                    <button onClick={() => setReviewRequest(r)}
                      style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--navy)', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      Review
                    </button>
                  )}
                </div>
        ...
        {tab === 'ledger' && (
          <div style={{ background: '#fff', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', fontWeight: 700, fontSize: 14 }}>Stock Movement Ledger</div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--bg)', fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase' }}>
                  <th style={{ padding: '9px 14px', textAlign: 'left' }}>Item</th>
              ...
              <tbody>
                {transactions.length === 0 ? (
                  <tr><td colSpan={5} style={{ padding: 30, textAlign: 'center', color: 'var(--text-4)' }}>No activity yet</td></tr>
                ) : transactions.map(t => (
                  <tr key={t.id} style={{ borderTop: '1px solid var(--surface-2)' }}>
                    <td style={{ padding: '9px 14px', fontWeight: 600 }}>{t.item?.name}</td>
                    <td style={{ padding: '9px 14px', textAlign: 'center' }}>{t.type}</td>
                    <td style={{ padding: '9px 14px', textAlign: 'center', fontWeight: 700, color: t.quantity < 0 ? 'var(--red)' : 'var(--green)', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                      {t.quantity < 0 ? <MdTrendingDown size={13} /> : <MdTrendingUp size={13} />} {t.quantity > 0 ? `+${t.quantity}` : t.quantity}
                    </td>
                    <td style={{ padding: '9px 14px', color: 'var(--text-3)' }}>{t.performedBy?.name}</td>
                    <td style={{ padding: '9px 14px', color: 'var(--text-3)' }}>{format(new Date(t.createdAt), 'dd MMM yyyy, h:mm a')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const actionBtn = { padding: '5px 10px', borderRadius: 6, border: '1px solid var(--border)', background: '#fff', color: 'var(--text-2)', fontSize: 12, fontWeight: 600, cursor: 'pointer' };
